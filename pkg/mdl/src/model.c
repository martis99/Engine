...
static Texture* read_texture(Texture* texture, Image* image, Renderer* renderer, Str texture_name, const char* path, int type) {
	if (texture_name.count == 0) {
		...
		Str file;
		str_create(&file, strlen(path) + texture_name.count + 1);
...
static Material* read_material(BData* data, Material* material, Image* diff_image, Texture* diff_texture, Image* spec_image, Texture* spec_texture, const char* path, Renderer* renderer, Shader* shader) {
	read_str(data);
...
Model* model_load(Model* model, Renderer* renderer, const char* path, const char* filename, Shader* shader, bool flipUVS, bool print) {
	Str file_path;
	str_create(&file_path, strlen(path) + strlen(filename) + 1);
	...
	model->objects = m_calloc(model->objects_count + 1, sizeof(ModelObject));
	...
	for (int i = 0; i < model->materials_count; i++) {
		read_material(&data, &model->materials[i], &model->images[i * 2], &model->textures[i * 2], &model->images[i * 2 + 1], &model->textures[i * 2 + 1], path, renderer, shader);
	}
...
void model_delete(Model* model, Renderer* renderer) {
	...
	m_free(model->objects, (model->objects_count + 1) * sizeof(ModelObject));
