...
static ID3D11Buffer* buffer_create_static(DX11Error* error, ID3D11Device* device, const void* data, UINT data_size, UINT element_size, D3D11_BIND_FLAG bindFlags) {
	...
	if (DX11_FAILED(error, "Failed to create static buffer", device->lpVtbl->CreateBuffer(device, &bd, &sd, &buffer))) {
		return NULL;
	}
...
static ID3D11Buffer* buffer_create_dynamic(DX11Error* error, ID3D11Device* device, UINT data_size, UINT element_size, D3D11_BIND_FLAG bindFlags) {
	ID3D11Buffer* buffer = NULL;
	...
	if (DX11_FAILED(error, "Failed to create dynamic buffer", device->lpVtbl->CreateBuffer(device, &bd, NULL, &buffer))) {
		return NULL;
	}
...
static void buffer_set_data(DX11Error* error, ID3D11Buffer* buffer, ID3D11DeviceContext* context, const void* data, SIZE_T data_size) {
	D3D11_MAPPED_SUBRESOURCE ms = { 0 };

	DX11_ASSERT(error, context->lpVtbl->Map(context, (ID3D11Resource*)buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &ms));
...
ID3D11Buffer* dx11_vb_create_static(DX11Error* error, ID3D11Device* device, const void* data, UINT data_size, UINT element_size) {
	return buffer_create_static(error, device, data, data_size, element_size, D3D11_BIND_VERTEX_BUFFER);
}

ID3D11Buffer* dx11_vb_create_dynamic(DX11Error* error, ID3D11Device* device, UINT data_size, UINT element_size) {
	return buffer_create_dynamic(error, device, data_size, element_size, D3D11_BIND_VERTEX_BUFFER);
}
...
void dx11_vb_set_data(DX11Error* error, ID3D11Buffer* vb, ID3D11DeviceContext* context, const void* data, SIZE_T data_size) {
	buffer_set_data(error, vb, context, data, data_size);
}
...
ID3D11Buffer* dx11_ib_create_static(DX11Error* error, ID3D11Device* device, const void* data, UINT data_size, UINT element_size) {
	return buffer_create_static(error, device, data, data_size, element_size, D3D11_BIND_INDEX_BUFFER);
}

ID3D11Buffer* dx11_ib_create_dynamic(DX11Error* error, ID3D11Device* device, UINT data_size, UINT element_size) {
	return buffer_create_dynamic(error, device, data_size, element_size, D3D11_BIND_INDEX_BUFFER);
}
...
void dx11_ib_set_data(DX11Error* error, ID3D11Buffer* ib, ID3D11DeviceContext* context, const void* data, SIZE_T data_size) {
	buffer_set_data(error, ib, context, data, data_size);
}
...
ID3D11Buffer* dx11_cb_create_static(DX11Error* error, ID3D11Device* device, const void* data, UINT data_size) {
	return buffer_create_static(error, device, data, data_size, 0, D3D11_BIND_CONSTANT_BUFFER);
}

ID3D11Buffer* dx11_cb_create_dynamic(DX11Error* error, ID3D11Device* device, UINT data_size) {
	return buffer_create_dynamic(error, device, data_size, 0, D3D11_BIND_CONSTANT_BUFFER);
}
...
void dx11_cb_set_data(DX11Error* error, ID3D11Buffer* cb, ID3D11DeviceContext* context, const void* data, SIZE_T data_size) {
	buffer_set_data(error, cb, context, data, data_size);
}
...
ID3D11InputLayout* dx11_il_create(DX11Error* error, ID3D11Device* device, D3D11_INPUT_ELEMENT_DESC* ied, UINT num_elements, ID3DBlob* blob) {
	...
	if (DX11_FAILED(error, "Failed to create input layour", device->lpVtbl->CreateInputLayout(device, ied, num_elements, blob->lpVtbl->GetBufferPointer(blob), blob->lpVtbl->GetBufferSize(blob), &layout))) {
		return NULL;
	}
	return layout;
}
