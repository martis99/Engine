...
ID3D11Texture2D* dx11_texture_create(DX11Error* error, ID3D11Device* device, AFilter filter, UINT width, UINT height, DXGI_FORMAT format, bool render_target, bool shader_resource, bool readable, const void* data, UINT row_size) {
...
ID3D11SamplerState* dx11_ss_create(DX11Error* error, ID3D11Device* device, AFilter filter, D3D11_TEXTURE_ADDRESS_MODE address) {
	...
	desc.Filter = dx11_afilter(filter);
