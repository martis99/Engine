...
GLuint gl_texture_create(GLError* error, GLint wrap, GLint filter, GLint width, GLint height, GLint internal_format, GLenum format, GLenum type, const GLvoid* data, bool generate_mipmaps) {
	...
	if (filter != 0) {
		if (GL_FAILED(error, "Failed to texture filter", glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, filter))) {
			return 0;
		}
		if (GL_FAILED(error, "Failed to texture filter", glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, filter))) {
			return 0;
		}
	}
