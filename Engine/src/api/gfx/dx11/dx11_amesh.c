...
AMesh* amesh_init_static(AMesh* mesh, ARenderer* renderer, AShader* shader, const void* vertices, uint vertices_size, uint* indices, uint indices_size, ADataType* layout, uint layout_size, APrimitive primitive) {
	...
	renderer->context->lpVtbl->IASetIndexBuffer(renderer->context, mesh->ib, DXGI_FORMAT_R16_UINT, 0);
	...
	hr = renderer->device->lpVtbl->CreateInputLayout(renderer->device, ied, 1, shader->vs_blob->lpVtbl->GetBufferPointer(shader->vs_blob), shader->vs_blob->lpVtbl->GetBufferSize(shader->vs_blob), &input_layout);
	if (FAILED(hr)) {
