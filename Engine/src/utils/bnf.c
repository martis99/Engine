...
#define BNF_THROW(call) if (call == BNF_FAILURE) { return BNF_FAILURE; }
#define BNF_THROW_IF(expression, message) if (expression) { bnf->error = message; return BNF_FAILURE; }
...
static void bnf_print_rule(Rule* rule) {
	...
	switch (rule->type)
	{
	case RULE_TYPE_VARIABLE_V:
		printf("$v");
		break;
	case RULE_TYPE_VARIABLE_I:
		printf("$i");
		break;
	}
