...
#define AST_ERROR(message)            \
	do {                          \
		ast->error = message; \
		return AST_FAILURE;   \
	} while (0)

#define AST_THROW(call)                     \
	do {                                \
		if (call == AST_FAILURE) {  \
			return AST_FAILURE; \
		}                           \
...
#define AST_THROW_IF(expression, message)     \
	do {                                  \
		if (expression) {             \
			ast->error = message; \
			return AST_FAILURE;   \
...
static int ast_parse_term(Ast *ast, AstNode *node, Term *term, Str *rule_name, int *add, Str *src)
{
	...
	case TERM_TYPE_LITERAL: return str_read_str(src, &term->text).count > 0 ? AST_SUCCESS : AST_END;
	...
	case TERM_TYPE_TAB: return str_read_char(src, '\t') != 0 ? AST_SUCCESS : AST_END;
	case TERM_TYPE_NEW_LINE: return str_read_char(src, '\n') != 0 ? AST_SUCCESS : AST_END;
	}
...
static int ast_gen_term(Ast *ast, AstNode *node, Term *term, Bnf *bnf, Str *dst)
{
	if (term->list == 1) {
		...
		switch (term->type) {
		case TERM_TYPE_RULE: return ast_gen_rule(ast, node, term->rule, term->optional, bnf, dst);
		case TERM_TYPE_LITERAL: str_add_str(dst, &term->text); return AST_SUCCESS;
		case TERM_TYPE_IDENTIFIER:
			if (node != NULL) {
		...
		case TERM_TYPE_FLOAT:
			...
			AST_ERROR("Number does not have node");
		case TERM_TYPE_TAB: str_add_char(dst, '\t'); return AST_SUCCESS;
		case TERM_TYPE_NEW_LINE: str_add_char(dst, '\n'); return AST_SUCCESS;
		}
...
void ast_print_node(AstNode *node, int depth)
{
	...
	str_print(&node->name);
	switch (node->type) {
	case ASTNODE_TYPE_PARENT: break;
	case ASTNODE_TYPE_LIST: printf(" list"); break;
	...
	case ASTNODE_TYPE_FLOAT:
		...
		str_print(&node->text);
		break;
	case ASTNODE_TYPE_VARIABLE_I: printf("(%i)", node->i); break;
	}
