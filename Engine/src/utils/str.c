...
{
	str_create_cstrn(str, cstr, count == 0 ? (uint)strlen(cstr) : count);
}
...
Str str_read(Str *str, int flag)
{
	...
	int count = 0;
	while (str->offset + count < str->count && ((space && str->data[str->offset + count] == ' ') || (new_line && str->data[str->offset + count] == '\n') ||
						    (null && str->data[str->offset + count] == '\0') || (alpha && isalpha(str->data[str->offset + count])) ||
						    (digit && isdigit(str->data[str->offset + count])) || (underscore && str->data[str->offset + count] == '_') ||
