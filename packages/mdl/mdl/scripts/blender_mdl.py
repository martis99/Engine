...
class Data:
    ...
    def object(self, object, parent_id, materials, all):
        ...
        rotationX = Matrix.Rotation(rotation.x * self.inv, 4, 'X')
        rotationY = Matrix.Rotation(rotation.y * self.inv, 4, 'Y')
        rotationZ = Matrix.Rotation(rotation.z * self.inv, 4, 'Z')
        scaleX = Matrix.Scale(scale.x, 4, (1, 0, 0))
        scaleY = Matrix.Scale(scale.y, 4, (0, 1, 0))
        scaleZ = Matrix.Scale(scale.z, 4, (0, 0, 1))
        transformation = translation @ rotationZ @ rotationY @ rotationX @ scaleZ @ scaleY @ scaleX
    ...
    def mesh(self, obj, mats):
            ...
            for vert_idx, loop_idx in zip(face.vertices, face.loop_indices):
                if vert_idx in uv_map:
                    if uvs[uv_map[vert_idx]].uv != uvs[loop_idx].uv:
                        self.report(f'{mesh.name} has different uv coordinates on the same vertex')
                        return {'FINISHED'}
                else:   
        ...
        for vertex in mesh.vertices:
                vp = vertex.co
                vn = vertex.normal
                vt = uvs[uv_map[vertex.index]].uv
                self.vec3(self.orient3 @ vp)
                self.vec3(self.orient3 @ vn)
                self.vec2(vt)

        self.int(len(polygons))
        for triangle in polygons:
            tr_indices = triangle.loop_indices
            if len(tr_indices) < 3:
                self.report(f'{mesh.name} contains faces with less than 3 vertices')
                select_face(obj, triangle)
                return {'FINISHED'}
            ...
            if self.operator.reverse == False:
                self.int(indices[tr_indices[0]].vertex_index)
                self.int(indices[tr_indices[1]].vertex_index)
                self.int(indices[tr_indices[2]].vertex_index)
            else:
                self.int(indices[tr_indices[0]].vertex_index)
                self.int(indices[tr_indices[2]].vertex_index)
                self.int(indices[tr_indices[1]].vertex_index)
    ...
    def parse_roughness(self, node, name):
        if len(node.links) == 0:
            ...
            self.report(f'{name} rougness is not supported')
            return None
...
def export_model(context, operator):
    print("exporting...")
...
class ExportModel(Operator, ExportHelper):
    ...
    def execute(self, context):
        return export_model(context, self)
