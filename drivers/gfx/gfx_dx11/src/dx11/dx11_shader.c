...
ID3D11VertexShader *dx11_vs_create(DX11Error *error, ID3D11Device *device, const char *src, ID3DBlob **blob)
{
	...
	if (DX11_FAILED(error, "Failed to compile vertex shader", D3DCompile(src, strlen(src), NULL, NULL, NULL, "vs_main", "vs_5_0", 0, 0, blob, &error_blob))) {
		if (error_blob) {
	...
	if (DX11_FAILED(error, "Failed to create vertex shader",
			device->lpVtbl->CreateVertexShader(device, (*blob)->lpVtbl->GetBufferPointer(*blob), (*blob)->lpVtbl->GetBufferSize(*blob), NULL, &vs))) {
		return NULL;
	}
...
ID3D11PixelShader *dx11_ps_create(DX11Error *error, ID3D11Device *device, const char *src, ID3DBlob **blob)
{
	...
	if (DX11_FAILED(error, "Failed to compile pixel shader", D3DCompile(src, strlen(src), NULL, NULL, NULL, "fs_main", "ps_5_0", 0, 0, blob, &error_blob))) {
		if (error_blob) {
			printf("%s\n", (char *)error_blob->lpVtbl->GetBufferPointer(error_blob));
		}
		return NULL;
	...
	if (DX11_FAILED(error, "Failed to create pixel shader",
			device->lpVtbl->CreatePixelShader(device, (*blob)->lpVtbl->GetBufferPointer(*blob), (*blob)->lpVtbl->GetBufferSize(*blob), NULL, &ps))) {
		return NULL;
	}
