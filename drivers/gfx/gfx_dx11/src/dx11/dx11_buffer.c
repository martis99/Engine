...
ID3D11InputLayout *dx11_il_create(DX11Error *error, ID3D11Device *device, D3D11_INPUT_ELEMENT_DESC *ied, UINT num_elements, ID3DBlob *blob)
{
	...
	if (DX11_FAILED(error, "Failed to create input layour",
			device->lpVtbl->CreateInputLayout(device, ied, num_elements, blob->lpVtbl->GetBufferPointer(blob), blob->lpVtbl->GetBufferSize(blob), &layout))) {
		return NULL;
	}
	return layout;
}
